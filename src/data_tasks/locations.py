...
for country in countries:
    if country not in loci:
        if len(country.split()) == 1:
            if country not in ",;.!%=/?+()":
                pattern = {"label": "GPE", "pattern": country}
        else:
            pattern_list = []
...
for city in hun_cities:
    if city not in loci:
        if len(city.split()) == 1:
            if city not in ",;.!%=/?+()":
                pattern = {"label": "GPE", "pattern": city}
        else:
            pattern_list = []
            for e in city.split():
                ed = {"lower": e.lower()}
...
df = pd.read_csv("data/loc/hungarian_geonames.tsv", sep="\t", encoding="utf-8")
    ...
    if location not in loci:
        if not pd.isnull(location) and not pd.isnull(locality_type):
            if (
                ...
                or locality_type == "village"
                or locality_type == "region"
                or locality_type == "city"
                or locality_type == "town"
                or locality_type == "national_capital"
            ):
                if len(location.split()) == 1:
                    if location not in ",;.!%=/?+()":
                        pattern = {"label": "GPE", "pattern": location}
                else:
                    pattern_list = []
